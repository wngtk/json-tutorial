...
> wngtk注：或许没有错误编码只是有的情况没有考虑到，使用assert()可以快速定位到问题。这个不合法参数也可能是用户的输入或者测试文件输入呢。积极使用assert()方便测试也方便调试。

> 学会使用assertion, 对程序的测试和调试都百利而无一害.


> 使用assert()设置检查点，拦截非预期情况。https://nju-projectn.github.io/ics-pa-gitbook/ics2022/1.5.html
> 你应该使用assert(0), 就算不"得体"地处理上述问题, 仍然不会影响表达式求值的核心功能的正确性

> 对于还未考虑到或者实现的部分，使用断言检查或者直接assert(0)，也可以方便定位所在。运行时的错误处理非常必要，但使用断言比写合适的错误处理要方便，使用断言不会影响程序的正确性。与其说断言是检测程序员编码错误，倒不如说断言在开发的过程中就应该积极使用，以拦截非预期的情况。
